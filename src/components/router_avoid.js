...
//
joint.routers.avoidBrokenArrows = function(vertices, opt, linkView) {
  ...
  var result = joint.routers.metro(vertices, opt, linkView);
  // console.log(JSON.parse(JSON.stringify(result)));
  return result;
  // return [sourcePoint].concat(vertices, targetPoint);
};

// итог: нужно или вернуть херню со стрелочкой смещенной
// или понять как был fallback для роутинга
// (если не взойдет - просто делать копипаст роута)
// и там при fallback делать соответствующий возврат
