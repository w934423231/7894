import { default as joint, g } from "jointjs/dist/joint.js";

joint.routers.manhattan2 = (function(g, joint, util) {
  var config = {
    ...
    fallbackRouter_origin: function(vertices, opt, linkView) {
      ...
      return joint.routers.orthogonal(
        vertices,
        util.assign({}, config, opt),
        linkView
      );
    },
    fallbackRouter: function(vertices, opt, linkView) {
      return joint.routers.modifedOneSide(
        vertices,
        util.assign({}, config, opt),
        linkView
      );
      ...
      if (dirList.includes(key)) {
        ...
        if (point) {
          ...
          if (bbox.containsPoint(point)) {
            round(
              point.offset(direction.x * grid.x, direction.y * grid.y),
              opt
            );
          }
    ...
    if (startPoints.length > 0 && endPoints.length > 0) {
      ...
      while (!openSet.isEmpty() && loopsRemaining > 0) {
        ...
        for (i = 0; i < numDirections; i++) {
          ...
          if (
            !(isPathBeginning && isStart) &&
            directionChange > opt.maxAllowedDirectionChange
          )
            continue;

          var neighborPoint = currentPoint
            .clone()
            .offset(direction.gridOffsetX, direction.gridOffsetY);
          ...
          if (endPointsKeys.indexOf(neighborKey) >= 0) {
            ...
            if (!isNeighborEnd) {
              var endDirectionAngle = getDirectionAngle(
                neighborPoint,
                end,
                numDirections,
                grid,
              ...
              var endDirectionChange = getDirectionChange(
                directionAngle,
                endDirectionAngle
              );
    ...
    for (var i = 0, len = oldVertices.length; i <= len; i++) {
      ...
      if (!to) {
        ...
        if (isEndingAtPoint && util.isFunction(opt.draggingRoute)) {
          ...
          partialRoute = opt.draggingRoute.call(
            linkView,
            dragFrom,
            dragTo,
            opt
      ...
      partialRoute =
        partialRoute || findRoute.call(linkView, from, to, map, opt);
